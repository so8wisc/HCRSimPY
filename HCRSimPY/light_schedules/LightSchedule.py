...
"""
This python module contains functions for creating light
schedules giving the light levels in lux as a function of
time in hours.
...
def interpolateLinear(t, xvals, yvals):
    """
    Implement a faster method to get linear
    interprolations of the light functions
    """
...
def interpolateLinearExt(t, xvals, yvals):
    """
    Implement a faster method to get linear interprolations
    of the light functions, exclude non-full days
    """
    i = np.searchsorted(xvals, t)-1
    ans = (yvals[i+1]-yvals[i])/((xvals[i+1]-xvals[i])*(t-xvals[i]))+yvals[i]
    return(ans)
...
def LightLog(lightlevel, threshold=1.0):
    """
    Take the log10 of the light levels,
    but map 0 to zero and not negative numbers
    """
...
def RegularLightSimple(t, Intensity=150.0, wakeUp=8.0, workday=16.0):
    """
    Define a basic light schedule with a
    given intensity of the light, wakeup time
    and length of the active period (non-sleeping)
    This schedule will automatically repeat on a
    daily basis, so each day will be the same.....
    """
...
def ShiftWorkLight(t, dayson=5, daysoff=2):
    """
    Simulate a night shift worker. Assume they
    are working a night shift for dayson number
    of days followed by daysoff normal days
    where they revert to a normal schedule
    ...
    if (t <= 24*dayson):
        ...
        return(RegularLightSimple(t, Intensity=150.0,
                                  wakeUp=16.0, workday=16.0))

    else:
        return(RegularLightSimple(t, Intensity=250.0,
                                  wakeUp=9.0, workday=16.0))


def SocialJetLag(t, weekdayWake=7.0, weekdayBed=24.0,
                 weekendWake=11.0, weekendBed=2.0):
    ...
    if (t <= 24*5):
        ...
        if duration < 0.0:
            duration += 24.0
        return(RegularLightSimple(t, Intensity=150.0,
                                  wakeUp=weekdayWake, workday=duration))
    if (t > 24*5 and t <= 24*7):
        ...
        if duration < 0.0:
            duration += 24.0
        return(RegularLightSimple(t, Intensity=250.0,
                                  wakeUp=weekendWake, workday=duration))
...
class WrightLightData(object):
    ...
    def __init__(self):
        ...
        self.art_data = np.array(sorted(map(tuple, list(np.loadtxt(
            '../ExpData/Wright/Wright_Artificial.csv', delimiter=','))),
            key=lambda x: x[0]))
        self.nat_data = np.array(sorted(map(tuple, list(np.loadtxt(
            '../ExpData/Wright/Wright_Natural.csv', delimiter=','))),
            key=lambda x: x[0]))
...
class NoisyLightInput(object):
    ...
    def __init__(self):
        ...
        # Light variability
        # Natural light standard deviation set at 10^ to this number
        self.nl_sd = 2.0
        # artificial light standard deviation
        self.al_sd = 50.0
    ...
    def generateTS(self):
        ...
        PP = 12.0
        IntLast = 0.0
        # This controls when the AP starts (defaults to dawn).
        # Can't do wake up earlier than dawn as of yet
        LightOnset = 0.0
        ...
        for t in self.time:
            ...
            # Sleep modulation of light input
            val = 0.5*sp.tanh(10*(fmod(t, period)-LightOnset)) - 0.5*sp.tanh(
                10*(fmod(t, period) - (AP+LightOnset)))
            if (fmod(t, period) <= PP):
                # Random light Intensity, allowing for natural light exposures
                Int = self.background_ll+10**np.random.normal(0.0, self.nl_sd)
            else:
...
class hchs_light(object):
    ...
    def __init__(self, filename, smoothWindow=20):
        ...
        self.data['TimeTotal'] = (self.data.day)*24.0+pd.DatetimeIndex(
            self.data.time).hour+pd.DatetimeIndex(
            self.data.time).minute/60.0+pd.DatetimeIndex(
            self.data.time).second/3600.0
        ...
        self.LightFunction = lambda t: interpolateLinear(fmod(
            t, self.endTime), np.array(self.data['TimeTotal']),
            np.array(self.data['Lux']))
        ...
        # Trim off the non-full days within the data set
        # and make a light function using that data

    def findMidSleep(self, cutoff=0.5, show=False):
        """Estimate the midpoint of the sleep.
        Actually the angular mean of the sleep times"""
        ...
        def toangle(x):
            return np.exp(complex(0, 1)*(2*sp.pi*x/24.0))
    ...
    def plot_light(self):
        """Make a plot of the light schedule
        with each day on top of the last"""
        ts = np.arange(
        ...
        """ plt.scatter(np.array(self.data.TimeTotal),
        map(LightLog, self.data.Lux), color='red', s=0.1)
        """
...
class JennyDataReader(object):
    ...
    def __init__(self, filename):
        ...
        fileData = pd.read_csv(Fullfilename, delimiter=',',
                               skiprows=3, parse_dates={
                                   'Date_Time': ['Date', 'Time']},
        ...
        self.data.loc[:, 'Sleep_Score'] = pd.Series(data=list(
            map(sleepscoref, self.data['Sleep'])),
            index=self.data.index, dtype=np.float64)
        ...
        self.data['TimeTotal'] = (days*24.0*60+pd.DatetimeIndex(
            self.data.index).hour*60+pd.DatetimeIndex(
            self.data.index).minute)/60.0

        # Store time bounds for ease of use
        self.startTime = self.data.TimeTotal.iloc[0]
        self.endTime = self.data.TimeTotal.iloc[-1]

        self.LightFunctionAvg = lambda t: interpolateLinear(t, np.array(
            self.avg_data.index), np.array(self.avg_data['Lux']), period=24.0)
        self.LightFunction = lambda t: interpolateLinear(
            t, np.array(self.data['TimeTotal']), np.array(self.data['Lux']))
        ...
        # Extract the dates to be used in comparing
        # with sleep data for included_days only
