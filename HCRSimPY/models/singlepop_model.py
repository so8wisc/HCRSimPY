...
Hannay, K. M., Booth, V., & Forger, D. B. (2019).
Macroscopic Models for Human Circadian Rhythms.
Journal of Biological Rhythms, 34(6), 658–671.
https://doi.org/10.1177/0748730419878298

Here are some marker states for that model
(all equals should be read as approx)
...
Note, this model rotates counterclockwise in accordance
with the usual convention in mathematics. The VDP family
of models rotate clockwise. This can be confusing when
trying to compare phase plane plots between the models,
but is fixable with a simple rotation.

An effort will be made to have the core methods align
between all of the models implemented
in this package.
...
class SinglePopModel(object):
    """
    A simple python program to integrate the human circadian
    rhythms model (Hannay et al 2019) for a given light schedule
    """

    def __init__(self, LightFun):
        """
        Create a single population model by passing in
        a Light Function as a function of time.

        This will create a model with the default parameter
        values as given in Hannay et al 2019.

        This class can be used to simulate and plot the
        results of the given light schedule on the circadian
        phase and amplitude.
        """
    ...
    def setParameters(self):
        """
        Load the model parameters, if useFile is False
        this will search the local directory for a
        optimalParams.dat file.
        ...
        """
        self.w0, self.K, self.gamma, self.Beta1, self.A1,
        self.A2, self.BetaL1, self.BetaL2, self.sigma, self.G,
        self.alpha_0, self.delta, self.p, self.I0 = [
            0.263524, 0.06358, 0.024, -0.09318, 0.3855, 0.1977,
            -0.0026, -0.957756, 0.0400692, 33.75, 0.05, 0.0075, 1.5, 9325.0]

    def updateParameters(self, paramDict):
        """
        Update the model parameters using a passed in
        parameter dictionary. Any parameters not included
        ...
        params = ['w0', 'K', 'gamma', 'Beta1', 'A1', 'A2', 'BetaL1',
    ...
    def getParameters(self):
        ...
        current_params = {'w0': self.w0, 'K': self.K, 'gamma': self.gamma,
                          'Beta1': self.Beta1, 'A1': self.A1, 'A2': self.A2,
                          'BetaL1': self.BetaL1, 'BetaL2': self.BetaL2,
                          'sigma': self.sigma, 'G': self.G,
                          'alpha_0': self.alpha_0, 'delta': self.delta,
                          'p': self.p, 'I0': self.I0}
    ...
    def updatePeriod(self, newVal):
        ...
        if (newVal >= 10.0 and newVal <= 35.0):
            ...
            print("The new circadian period should be in hours," +
                  " it looks like you forgot this so the period was" +
                  " not updated.")

    def alpha0(self, t):
        """A helper function for modeling the light input processing"""
        assert self.Light(t) >= 0.0
        return(self.alpha_0*pow(self.Light(t),
                                self.p)/(pow(self.Light(t), self.p)+self.I0))
    ...
    def derv(self, t, y):
        ...
        LightAmp = self.A1*0.5*Bhat*(1.0-pow(R, 4.0))*sp.cos(
            Psi+self.BetaL1)+self.A2*0.5*Bhat*R*(1.0-pow(
                R, 8.0))*sp.cos(2.0*Psi+self.BetaL2)
        LightPhase = self.sigma*Bhat-self.A1*Bhat*0.5*(pow(
            R, 3.0)+1.0/R)*sp.sin(
                Psi+self.BetaL1)-self.A2*Bhat*0.5*(1.0+pow(
                    R, 8.0))*sp.sin(2.0*Psi+self.BetaL2)
    ...
    def integrateModel(self, tend, initial=[1.0, 0.0, 0.0]):
        ...
        The parameters are tend= the end time to stop the
        simulation and initial=[R, Psi, n]

        Writes the integration results into the scipy array self.results.

        Returns the circadian phase (in hours) at the ending
        time for the system.
    ...
    def integrateModelData(self, timespan, initial, dt=0.1):
        ...
        initial[1] = fmod(initial[1], 2*sp.pi)
        r = sp.integrate.solve_ivp(
            self.derv, (timespan[0], timespan[1]),
            initial, t_eval=self.ts, method='Radau')
    ...
    def integrateTransients(self, numdays=50):
        ...
        r = sp.integrate.solve_ivp(self.derv, (0, tend), [0.7, 0.0, 0.01],
                                   t_eval=[tend], method='Radau')
    ...
    def getTS(self, addMelatonin=True):
        ...
        Has a very, very simple melatonin state prediction
        which is off by default

        getTS(self, addMelatonin=True)

        returns a pandas data frame with the Time,
        Light_Level in lux, Phase (radians), R (amplitude),
        n (light activation variable) as columns
        """

        light_ts = list(map(self.Light, self.ts))
        ts = pd.DataFrame({'Time': self.ts, 'Light_Level': light_ts,
                          'Phase': self.results[:, 1], 'R': self.results[:, 0],
                           'n': self.results[:, 2]})
        ...
        if (addMelatonin):
            ...
            for i in range(self.results.shape[0]):
                ...
                if ((phase >= 1.309) and (phase <= 3.92) and
                        (light_ts[i] <= light_threshold)):
    ...
    def findKeyTimes(self):
        """Find the mean circadian phases at different
        times in the data set as well as the variation.

        findKeyTimes()

        Returns a pandas data frame with the circular mean
        of the phase coherence at each time point

        """
        wrapped_time = np.round([fmod(x, 24.0) for x in self.ts], 2)
        df = pd.DataFrame({'Time': wrapped_time, 'Phase': self.results[:, 1]})

        # Find the circular statistics for the circadian
        # phase data at each time point
        ...
        mean_func = sp.interpolate.interp1d(
            np.array(df2['Circular_Mean']), np.array(df2.index))
    ...
    def findAveragePhase(self):
        ...
        Returns a pandas data frame with index given by
        the wrapped time, the mean phase across the simulation,
