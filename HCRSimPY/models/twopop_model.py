...
Note, this model rotates counterclockwise in
accordance with the usual convention
in mathematics. The VDP family of models rotate
clockwise. This can be confusing when trying
to compare phase plane plots between the models,
but is fixable with a simple rotation.

An effort will be made to have the core methods
align between all of the models implemented
in this package.
...
class TwoPopModel(object):
    """ A simple python implementation of the two
    population human model from Hannay et al 2019"""

    def __init__(self, LightFun):
        """
        Create a two population model by passing in a
        Light Function as a function of time.

        This will create a model with the default parameter
        values as given in Hannay et al 2019.

        This class can be used to simulate and plot the results
        of the given light schedule on the circadian phase
        and amplitude.
        """
    ...
    def setParameters(self):
        """
        Sets the model parameters to the default
        published values. These are stored as
    ...
    def updateParameters(self, paramDict):
        """
        Update the model parameters using a passed
        in parameter dictionary. Any parameters not included
        ...
        params = ['tauV', 'tauD', 'Kvv', 'Kdd', "Kvd",
                  "Kdv", 'gamma', 'A1', 'A2', 'BetaL', 'BetaL2',
                  'sigma', 'G', 'alpha_0', 'delta', 'p', 'I0']
    ...
    def getParameters(self):
        ...
        current_params = {'tauV': self.w0, 'tauD': self.tauD,
                          'Kvv': self.Kvv, 'Kdd': self.Kdd, 'Kdv': self.Kdv,
                          'Kvd': self.Kdv, 'gamma': self.gamma, 'A1': self.A1,
                          'A2': self.A2, 'BetaL': self.BetaL,
    ...
    def alpha0(self, t):
        ...
        return(self.alpha_0*pow(self.Light(t), self.p)/(pow(
            self.Light(t), self.p)+self.I0))
    ...
    def derv(self, t, y):
        ...
        LightAmp = self.A1*0.5*Bhat*(1.0-pow(Rv, 4.0))*cos(Psiv+self.BetaL) + \
            self.A2*0.5*Bhat*Rv*(1.0-pow(
                Rv, 8.0))*cos(2.0*Psiv+self.BetaL2)
        LightPhase = self.sigma*Bhat-self.A1*Bhat*0.5*(pow(
            Rv, 3.0)+1.0/Rv)*sp.sin(
            Psiv+self.BetaL)-self.A2*Bhat*0.5*(1.0+pow(
                Rv, 8.0))*sp.sin(2.0*Psiv+self.BetaL2)

        dydt = np.zeros(5)

        dydt[0] = -self.gamma*Rv+self.Kvv/2.0*Rv * \
            (1-pow(Rv, 4.0))+self.Kdv/2.0*Rd * \
    ...
    def integrateModel(self, tend, initial=[1.0, 1.0, 0.0, 0.0, 0.0]):
        ...
        Returns the circadian phas of the ventral SCN
        (in hours) at the ending time for the system.
    ...
    def integrateTransients(self, numdays=50):
        ...
        r = sp.integrate.solve_ivp(self.derv, (0, tend),
                                   [0.7, 0.7, 0.0, 0.0, 0.0],
    ...
    def getTS(self):
        ...
        Light_Level in lux, Phase ventral (radians),
        Rv (amplitude), n (light activation variable) and phase
        ...
        ts = pd.DataFrame({'Time': self.ts, 'Light_Level': light_ts,
                          'Phase': self.results[:, 2], 'R': self.results[:, 0],
                           'n': self.results[:, 4],
                           'theta': self.results[:, 2]-self.results[:, 3]})
        return(ts)
